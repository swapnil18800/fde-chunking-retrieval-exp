...
Judge: `--judge gemini` round-robins over `GEMINI_MODELS` (`pipeline/llm.py::RoundRobinChat`) but is
quota-bound (~37 s/sample measured); `--judge deepseek` was used for the reported run (~3 min per config). Generation model for the
...
### 4.5 Tier 2 — RAGAS answer quality (`ragas_eval150_top4`, 2026-09-18)

First 40 questions of `eval150`, `top_k=5` context, answers generated by Gemini (`gemini-3.5-flash-lite`
round-robin). **Judge: `deepseek-chat`** — the free-tier Gemini judge ran at 37 s/sample (≈2 h per config,
23 quota retries in 15 min), so this is the one place DeepSeek was used (≈$0.5 per config). Retrieval-only
columns (`recall@5`, `mrr`) are exact; `id_based_context_*` are exact too (PMIDs); the other three are
LLM-judged. Raw tables + per-question answers: `evals/results/20260918T073420Z_ragas_eval150_top4/`.
...
1. **Answers are grounded**: faithfulness 0.90–0.93 for every config — Gemini rarely asserts something
   the retrieved abstracts do not say. Answer relevancy is uniformly high (0.86–0.88).
2. **Factual correctness tracks retrieval recall**, not prompt or model: `passage+bm25+rerank` has the
   best context recall (0.566) and the best factual F1 (0.447); HyDE has the lowest recall on this slice
   (0.515) and the lowest F1 (0.37). The BioASQ reference answers are dense lists of entities; the
   generator can only reproduce what its top-5 contains. **Retrieval is the ceiling.**
3. **Reranking is the best end-to-end lever** (+0.027 factual F1, +0.037 context recall, MRR 0.852)
   for +0.6 s. Its faithfulness dip (0.934 → 0.896) is small and consistent with richer context giving
   the model more to paraphrase.
4. **HyDE's retrieval MRR win does not survive Tier 2.** The hypothetical abstract sharpens the first hit
   but pulls the rest of the top-5 toward the LLM's own guess; recall and F1 drop and latency doubles
   (11 s). Use it for "find me the one paper", not for list answers.
5. **Sentence-window context is the most *precise* context (0.644 id-context-precision, +0.14 over
   `passage`)** at the cost of recall (0.501): five sentences ± neighbours cover fewer distinct
   abstracts than five whole abstracts. Faithfulness (0.917) and relevancy (0.873) stay high, so for
   users who read citations, fine chunks + window give tighter, more checkable evidence; for list
   questions, whole abstracts win.
6. Caveat: 40 questions, one judge run — differences under ~0.03 on LLM-judged metrics are noise.
   `id_based_*`, `recall@5`, `mrr` are exact and agree with the 150-question Tier-1 results.
